...
namespace amath {
	...
	class plm
	{
		// ¬ажно! Ёлементы полинома идут от нулевой степени и выше слева на право, а не наоборот как обычно. 
	private:
		double* koefs = 0; // 
		int degree = 0; // —тепень указываетс€ на 1 еденицу больше, тоесть если степень полинома 4 то  degree = 5, так как у нас существует ещЄ нулева€ степень
		int print_s = 0;
	...
	public:
		...
		friend plm operator+(plm& pol1, plm& pol2) {
			int mn, mx;
			double* koefs_nw;
			if (pol1.degree == pol2.degree) { // ≈сли степени полиномов равны, то провер€ем на обнуление высшей степени
				mx = pol1.degree;
				for (int i = 1; i < mx; i++) {
